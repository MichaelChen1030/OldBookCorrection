...
for filename in glob.glob('freq_output_final/*.xml'):
	with open("freq_output_final2/" + filename.split(slash1)[1].split(".")[0] + ".xml", "w", encoding='UTF-8', newline='') as write_file:
		with open(filename, "r", encoding='UTF-8', newline='') as read:
			for i in read:
				if re.match(".*lemma=.*", i):
					word = i.split("</w>")[0].split(">")[1]
					...
					if len(text_list) > 0 and probabilities[count1]>=0.1:
...
with open("freq_output/supplemental.csv","a", encoding='UTF-8') as output:
...
counterx=0
with open("freq_output/supplemental.csv", "r", encoding='UTF-8') as output:
	with open("freq_output/supplemental1.csv","w", encoding='UTF-8') as handle:
		...
		for i, line in enumerate(reader):
			...
			if str(line)=="[]":
				...
				handle.write(templine)
				handle.write("\n")
os.remove("freq_output/supplemental.csv")
